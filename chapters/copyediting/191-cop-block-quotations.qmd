...
> I am happy. [@doe_j:2020]

> I am sad.[^191-cop-block-quotations-1]
...
You should distinguish *several consecutive quotations* from *a single
quotation with several paragraphs*. One way to do this is as follows:
...
>
> ```bash
> @echo "this code block is a further block in the second quotation"
> ```
...
That is, we start all lines of the long quotation with `>`, *including
the blank lines that separate its paragaphs or blocks*. By contrast, we
leave an empty line between two distinct quotations.

An alternative is instead to only have `>` at the beginning of a long
quotation and indicate with subsequent paragraphs with an indent:
