...
When an article cross-references its own sections, figures, tables, images,
numbered examples, equations and footnotes, we encode these in markdown. This
ensures that they are linked (and, in some case, standardly labelled and
properly numbered) in the outputs.

Cross-references to sections, figure, tables, images are handled by the
...
Cross-references to numbered examples is handled by `pandoc` itself, usage
described below.
...
manually specified identifiers but also automatically generates identifiers. A manually specified identifiers is given by adding `{#sec:mysecid}` after the heading. An automatically generated identifier is obtained by putting the section heading in
lowercase, removing all formatting, links, footnotes, removing all characters
other than letters, numbers, underscore, periods and hyphens, and replacing
spaces by hyphens. Some examples:
...
As the last line illustrates, you need to repeat the custom prefix to each
...
Tables, images can be cross-referenced and can be given a coption
...
* But **do not* place an empty Span right at the beginning of the
  foonote: in this position the Span leaves an unwanted blank space:
...
you will need to update crossreferences to footnotes. 
...
> 1. first item
> 2. []{#cross-referenced-item}section item
> 3. third item.
> 
> My favour item is ([2](#cross-reference-item)), what's yours?
...
entered in the link, not automatic, so you have to make sure you know
which number it is.
...
If you want a part of the text to link to another, say have a statement labelled
...
As [@my-new-claim] established, ... .
but with a much cleaner syntax behind.
