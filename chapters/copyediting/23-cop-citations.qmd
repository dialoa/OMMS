...
This chapter details how to enter citations in Pandoc markdown.
If you're using a visual editor for Quarto or Pandoc (RStudio
or Visual Code extension) you can also enter citations and
select their style using the editor.
...
### Locators: ibid., Op. Cit., sec. or § and the like

**The template decides what the locators are**.
...
**Exceptions: how to override defaults**. Exceptionally you may need to override
the default locators. For instance, the section locator 'sec.' might look odd 
when citing historical works traditionally referred to using '§'. In those 
*exceptional* cases, ensure that your locators are printed as is by preceeding 
them with an invisible 'zero-width non-joiner' charater (`&zwnj;'): 

``` markdown

See @frege:1967a [&zwnj;§11]. Many agree [@smith, &zwnj;pp. 56-58].
